...
use AiCrawler\Support\Exceptions\HeuristicConventionViolatedException;
use AiCrawler\Support\Exceptions\HeuristicDoesNotExistException;
use AiCrawler\Support\Exceptions\HeuristicsNotDefinedException;
...
abstract class AbstractScraper {
    ...
    public function scrape(){
        if (count($heuristics = $this->getHeuristics())) {
            foreach($heuristics as $context => $class) {
                ...
                elseif (class_exists("\\AiCrawler\\Heuristics\\".$class))
                    self::bfs(
                        $this->html,
                        $context,
                        ["\\AiCrawler\\Heuristics\\" . $class, "score"]
                    );
                else
    ...
    public function autoGetSanitizers() {
        ...
        array_walk($this->getHeuristics(), function($value, $key) use(&$validSanitizers) {
            ...
            elseif(class_exists("\\AiCrawler\\Sanitizers\\".$s))
                $validSanitizers[$key] = "\\AiCrawler\\Sanitizers\\".$s;
        });
